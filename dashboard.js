...
const dashboard = {
    ...
    syncDB: async function (silent = false) {
        ...
        if (studentsRaw && studentsRaw.length > 0) {
            window.schoolDB.students = studentsRaw.map(s => {
                ...
                if (s.sections) {
                    divisionName = s.sections.name || 'N/A';
                    if (s.sections.classes) {
                        className = s.sections.classes.name || 'N/A';
                    }
                }
                
                // Fallback to legacy columns
                if (className === 'N/A') className = s.class || 'N/A';
                if (divisionName === 'N/A') divisionName = s.division || 'N/A';

                // Handle composite class names (e.g. "7th-B")
                if (className.includes('-') && (divisionName === 'N/A' || !divisionName)) {
            ...
            console.log("DB students empty or failing, generating robust mock fallback...");
            window.schoolDB.students = this._generateMockStudents();
        }
        ...
        if (examsRaw) {
            ...
            window.schoolDB.exams = examsRaw.map(e => ({
                id: e.id,
                title: e.title,
                class: classMap[e.class_id] || e.class || 'All',
                date: e.start_date,
    ...
    // Normalize class name: '8th' -> '8th', 'Grade 8' -> '8th', '7th-B' -> '7th'
    normalizeClassName: function (className) {
        if (!className) return '';
        // Strip "Grade " prefix, then split by dash or space and take the first portion
        return className.toString().replace(/^Grade\s+/i, '').split(/[\s-]/)[0].trim();
    },
    ...
    // --- MOCK DATA GENERATORS (SAFE FALLBACKS) ---
    _generateMockStudents: function () {
        const classes = ['Nursery', 'LKG', 'UKG', '1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th', '9th', '10th'];
        const divisions = ['A', 'B'];
        const students = [];
        const names = ['Aryan Sharma', 'Zia Khan', 'Ishaan Verma', 'Ananya Gupta', 'Rohan Das', 'Sana Parveen', 'Kabir Singh', 'Meera Iyer', 'Arjun Reddy', 'Kyra Malhotra'];

        classes.forEach(cls => {
            divisions.forEach(div => {
                for (let i = 1; i <= 5; i++) {
                    const id = `STD-${cls}-${div}-${i}`;
                    students.push({
                        id: id,
                        db_id: id,
    ...
    _generateMockFees: function () {
        const students = schoolDB.students || [];
        const fees = [];
        const classStats = {}; // Track count per class to aim for 5 Paid / 5 Pending

        students.forEach(s => {
            if (!classStats[s.class]) classStats[s.class] = { paid: 0, pending: 0 };
            
            let status = 'Pending';
            if (classStats[s.class].paid < 5) {
                status = 'Paid';
                classStats[s.class].paid++;
            } else {
    ...
    triggerFeeRecoveryFlow: async function (pendingFees) {
        if (pendingFees.length === 0) return;
        
        const webhookUrl = 'https://studio.pucho.ai/api/v1/webhooks/4JqnOMaZ4qeTHElL9FyT1';
        const payload = {
            action: 'FEE_RECOVERY_INITIATED',
            timestamp: new Date().toISOString(),
            school_name: 'Pucho Academy',
            total_pending_count: pendingFees.length,
            records: pendingFees.slice(0, 50).map(f => ({
                student: f.student_name,
                class: `${f.class}-${f.division}`,
                amount: f.amount,
                parent_phone: '9876543210' // Placeholder for security in mock
            }))
        };

        try {
            console.log("[Fee Recovery] Triggering Webhook flow...");
            fetch(webhookUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            ...
            console.error("[Fee Recovery] Webhook Error:", e);
        }
    }
};
